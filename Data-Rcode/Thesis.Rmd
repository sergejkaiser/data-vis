...
library(boot)
library(extrafont)
setwd("/Users/sergej/blog/data-vis/Data-Rcode")
opts_chunk$set( cache.path = "/Users/sergej/blog/data-vis/Data-Rcode/Thesis_cache/", 
    fig.width = 10, fig.height = 8, message = FALSE, error = FALSE, warning = FALSE, echo=F,
    echo = TRUE, dev = "svg")
...
\begin{equation} \label{eq:1}\ln \left( \frac{x_{i,j}^k x^{k'}_{i'j}}{x_{i,j}^{k'} x^{k}_{i'j}} \right)= \theta \ln \left( \frac{z_{i}^k z^{k'}_{i'}}{z_{i}^{k'} z^{k}_{i'}} \right)-\ln \left( \frac{ d_{i,j}^k d^{k'}_{i'j}}{d_{i,j}^{k'} {d}^{k}_{i',j}} \right) 
(\#eq:relative-exports)
\end{equation}
...
Under the additional assumption that the trade cost in all three cases can be modeled as the product of a bilateral trade cost and an importer-sector trade cost, the empirical specification of \@ref(eq:relative-exports) is as follows.
...
The authors state that the following simpler equation may be estimated equivalently instead of the previous equation.
  ...
  (\#eq:bi-exports)
 \end{align} 
The \@ref(eq:bi-exports) states that the three trade indicators EXGR, FVAT and BVAT $x_{i,j}^k $ from country $i$ in sector $k$ to market $j$ are determined by the inverse of production cost $\ln z_i^k$, exporter-importer fixed-effects $\delta_{i,j}$ and importer-sector fixed-effects $\delta_j^k$. 
...
It can be inferred on the basis of \@ref(eq:3) that  $\theta$ may be directly estimated using value-added trade or EXGR.
...
colors2<-list(c("#e41a1c","#377eb8"),c("#4daf4a","#984ea3"),
              c("#e41a1c","#377eb8"),c("#984ea3","#4daf4a"))
  ...
  plot(ggplot(df, aes(x=IND, y=VAX)) +
        ...
        # scale_color_brewer(name="",palette = "Dark2") +
         ...
         legend.position = "bottom", # legend location in graph
                  legend.background = element_blank(),axis.title.x = element_blank(),
...
grid.arrange(p.3.2.1,p.3.2.2,ncol=2)
```
...
Then I extend the definition of eigenvector centrality to the weighted international trade network.^[I follow the outline of @bonacich2001evcent, who attribute the original exposition to @bonacich1972.]
 <br>
The centrality $c^e_{j'}$ of a node $j'$ is proportional to the weighted sum of the centrality of the nodes it is connected to. \begin{align} \lambda c^e_{j'} &= g_{1,j'} c^e_{1}+g_{2,j'} c^e_{2}+ \dots + g_{n,j'} c^e_{n} 
(\#eq:centrality-general)
\end{align} where $\lambda$ denotes a proportionality constant. 
The equation \@ref(eq:centrality-general) may be restated in matrix notation as below. It is the general eigenvector equation and  it  has $n$ solutions for $n$ values of $\lambda$. 
\begin{align}
\label{eq:6}
\boldsymbol{G}^T \boldsymbol{C}^e = \boldsymbol{C}^e  \boldsymbol{\lambda}
(\#eq:centrality-general-matrix)
\end{align}
...
Accordingly, equation \@ref(eq:centrality-general-matrix) simplifies to the following equation, where $\boldsymbol{c}^e$ denotes the principal eigenvector.
\begin{align}
\label{eq:17}
...
tiva.2005.agg.cou.par<-tiva.2005.sub.final %>% filter(.,VAR%in%c("FFD_DVA")) %>% group_by(COU,PAR) %>% summarise(sum.fvat.ind=sum(Value))
 ...
 p.1.out.eigen.cou.par<- ggplot(eigen.res.cou.par.tb, aes(reorder(COU, -out.eigen.cent),out.eigen.cent,fill=cor.pres)) +
   ...
   labs(x="",y="Out eigenvector centrality",title="")+
   scale_fill_manual(values=c("#999999","#000000"))+
    #scale_fill_brewer(type="seq",palette =6)+
   # scale_color_brewer()+
   coord_cartesian(ylim=c(0,0.7))+
   scale_x_discrete( expand = c(0.00, 0.00))+
    scale_y_continuous(breaks=c(seq(0,0.6,0.1)),minor_breaks=c(seq(0.05,0.6,0.1)), expand = c(0.02, 0.00))+
    theme_minimal()+
  theme(axis.title.x = element_blank(),
              panel.grid.major.x = element_blank(),
              panel.grid.minor = element_blank(),
