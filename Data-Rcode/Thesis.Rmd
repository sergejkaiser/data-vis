...
opts_chunk$set( cache.path = "cache", 
    fig.width = 8, fig.height = 6, message = FALSE, error = FALSE, warning = FALSE, echo=F,
    echo = TRUE, dev = "svg")
my.ggplot2<-function(df,title,shape.val,linetype.vals,color){ 
  ggplot(df, aes(x=IND, y=VAX)) +
          ...
          labs(x="Industry",y="RCA",title=title)+
          theme_bw() +   
...
Economics from the KU Leuven. Especially, I want to thank my promotor Liza Archanskaia for her comments and discussions, which helped me to advance this thesis. Leuven, 16/01/2017.
...
^[[@Timmer2015] analyzed 560 final manufacturing products in 40 countries of which 27 were European countries.] <br>
...
To give an example, [@Koopman] show that the trade surplus of China with the USA is reduced by 41  perc. based on value-added trade compared to gross exports. <br>
...
The first objective of this thesis is to evaluate whether the pattern of revealed comparative advantage changes significantly, when it is computed on the basis of domestic factors only.<br>
...
I assess the degree of divergence of revealed comparative advantage rankings obtained on the basis of gross exports and of value-added trade with two measures of association: Spearman's $\rho$ and Kendall's $\tau$. <br>
...
But there are divergences when I compare gross exports to forward value-added trade.<br>
...
The dissimilarity of the ranking for gross exports and forward value-added trade trade indicates that comparative advantage according to the factor content of trade differs from comparative advantage according to the domestic supply chain. <br>
...
However, it does not capture very well the pattern of comparative advantage associated with the factor content of trade.<br> 
...
The shift of focus is necessary because I want to correctly capture how shocks to domestic production factors diffuse through the international trade network. <br>
...
Elements of the eigenvector capture the contribution of each node when an extra dollar is added to the network [@spizzirri2011]. <br>
...
Consequently, it may be expected that both measures capture the ranking of relative sectoral ability of any given pair of countries.<br>
...
The latter approach is simpler because the ranking is directly obtained from value-added trade and does not rely on estimation.  <br>
...
I contribute to this literature by switching from the ad-hoc measure of revealed comparative advantage to the structural measure derived by [@costinot]. <br>
...
$L_i$ denotes the number of workers in each country $i$  and $ w_i $ denotes their wage. <br>
%The goods are produced using a production technology, which is modeled as follows. 
...
The productive efficiency  $z^k_i(\omega)$ describes how many units of the variety $\omega$ of good $k$ can be produced with one unit of labor in country $i$. <br> 
^[CDK showed in the working paper version that the close link between trade flows and productivity differences holds as well under weaker assumptions as long as the technology differences across countries are small.]
The assumption that the productivity distribution is Frechet follows the seminal article of [@eaton].
...
The dispersion parameter is assumed to be the same across sectors and countries. <br> 
...
denotes the trade cost of sector $k$ in country $i$ exporting to country $j$.  <br>
An auxiliary assumption about the trade cost is that the triangle inequality holds. 
This means that for any third country $j$, it is more expensive to indirectly import a good from country $i$ through country $i'$ than to directly import the good.  <br>
...
The price for each variety of a good is equal to the cost of production $\frac{w_i}{z_i^k}$  multiplied by the transport cost $d^k_{i,j}$.
...
The restriction on $\sigma^k_j$ is a technical assumption. It guarantees the existence of a well-defined CES price index $p_j^k$.  <br>
...
Thus, the contributions of inputs cancel out in relative terms and only the contribution of domestic productivity is left to determine the ranking of sectoral exports. <br>
...
Global input-output tables are linked by researchers on the basis of national input-output tables, which are constructed by national statistical agencies [@timmer_gvc].   <br> 
...
On the basis of the flow of gross output, VAT is obtained by multiplying the flows of gross output with the ratio of the value-added to gross output at the bilateral sectoral level. <br>
...
Under the additional assumption that the trade cost in all three cases can be modeled as the product of a bilateral trade cost and an importer-sector trade cost, the empirical specification of eq 2.1 is as follows.
...
The authors state that the following simpler equation may be estimated equivalently instead of the previous equation.
 ...
 \end{align} 
The eq. 2.3 states that the three trade indicators EXGR, FVAT and BVAT $x_{i,j}^k $ from country $i$ in sector $k$ to market $j$ are determined by the inverse of production cost $\ln z_i^k$, exporter-importer fixed-effects $\delta_{i,j}$ and importer-sector fixed-effects $\delta_j^k$. 
This approach is akin to a 'difference-in-difference' estimation. <br>
The structural revealed comparative advantage measure is based on the estimated exporter-sector fixed effect.
The fixed effect $\delta_i^k$. is the empirical equivalent to the $\theta \ln z_i^k$ term in eq. 2.2.   
...
It can be inferred on the basis of eq. 2.4 that  $\theta$ may be directly estimated using value-added trade or EXGR.
...
^[A sector denotes one or more chapter of the ISIC REV. 3.1 classification.] <br>
...
It covers 19 sectors, of which 6 are service sectors, 11 are manufacturing sectors and 2 are primary sectors.^[A sector denotes one or more chapter of the ISIC REV. 3.1 classification.] <br>
...
```{r fig data prep, message=FALSE, warning=FALSE, cache=T, include=FALSE}
...
```{r data prep, echo=FALSE, message=FALSE, warning=FALSE, cache=T}
...
colors2<-list(c("#e41a1c","#377eb8"),c("#4daf4a","#984ea3"),
              c("#377eb8","#e41a1c"),c("#984ea3","#4daf4a"))
...
```{r prep plots, message=FALSE, warning=FALSE, cache=T, include=FALSE}
list.of.titles<-list("RCA across industries of Belgium realtive to Germany WiOD","Structural RCA across industries for Belgium WiOD","Structural RCA across industries for Germany WiOD","Structural RCA across industries for Belgium  WiOD","Structural RCA across industries for Germany WiOD")
list.of.shape.vals<-list(c(1,2,1,2),c(4,1),c(4,1), c(3,1),c(3,1))
...
dat.m.wiod.2.bel<-filter(z.long.exgr.fddva.dva.wiod,COU %in% "BEL") %>% filter(.,Variable %in% c("EXGR","DViX"))
dat.m.wiod.2.deu<-filter(z.long.exgr.fddva.dva.wiod,COU %in% "DEU")%>% filter(.,Variable %in% c("EXGR","DViX"))
dat.m.wiod.1.bel<-filter(z.long.exgr.fddva.dva.wiod,COU %in% "BEL")%>% filter(.,Variable %in% c("EXGR","DVA"))
dat.m.wiod.1.deu<-filter(z.long.exgr.fddva.dva.wiod,COU %in% "DEU")%>% filter(.,Variable %in% c("EXGR","DVA"))

list.of.df<-list(dat.m.wiod.2.bel,dat.m.wiod.2.deu,dat.m.wiod.1.bel,dat.m.wiod.1.deu)

list.of.titles<-list("Structural RCA across industries for Belgium WiOD","Structural RCA across industries for Germany WiOD","Structural RCA across industries for Belgium  WiOD","Structural RCA across industries for Germany WiOD")

list.of.titles<-c(rep("",4))

list.of.shape.vals<-list(c(4,1),c(4,1), c(3,1),c(3,1))

list.of.linetype.vals<-list(c(1,2),c(1,2),c(4,1),c(4,1))

colors2<-list( c("#e41a1c", "#377eb8"), c("#1b9e77", "#d95f02"),c("#377eb8", "#e41a1c"), c( "#d95f02","#1b9e77"))
list.of.plots<-pmap(list(df=list.of.df,title=list.of.titles,shape.val=list.of.shape.vals,linetype.vals=list.of.linetype.vals,color=colors2),function(df,title,shape.val,linetype.vals,color){ 
...
```
```{r cor rca wiod, cache=T, warning=F,message=F,echo=F,cache.lazy=FALSE}
###################################################COR RCA WIOD ####################################################################
small.res<-list()
...
p1.2.1<-list.of.plots[[4]]$plot
p1.2.2<-list.of.plots[[3]]$plot
```
...
As stated earlier, the results are robust to the specific value of $\theta$.  <br>
...
Especially, if the RCA is larger than one it indicates that the country-sector productivity scaled by the sample mean is larger than the expected value for the country-sector, which is the product of the country mean and sector mean  [@leromain2014]. 
...
```{r fig-7 Country pair RCA: BVAT and EXGR, echo=FALSE, fig.align='center', fig.cap="Country pair RCA: BVAT and EXGR", message=FALSE, warning=FALSE}
grid.arrange(p1.2.1,p1.2.2,ncol=2)
```
...
```{r figure 2.2 Strength of association: BVAT EXGR RCA, cache=T,include=F}
...
```{r plot figure 2.2, echo=FALSE, fig.align='center', fig.cap='Strength of association: BVAT EXGR RCA', message=FALSE, warning=FALSE}
...
I then discuss the results for the full set of countries. <br>
...
However, for Belgium, I observe the opposite pattern being picked up in the following four sectors: 'wood products'  (20), 'paper and paper products' (21-22), 'machinery' (29). <br>
...
```{r Country pair RCA: EXGR and FVAT RCA, echo=FALSE, fig.align='center', fig.cap='Country pair RCA: EXGR and FVAT RCA'}
p.3.2.1<-list.of.plots[[1]]$plot
p.3.2.2<-list.of.plots[[2]]$plot
grid.arrange(p.3.2.1,p.3.2.2,name="Country pair RCA: EXGR and FVAT RCA",ncol=2)
```
...
Specifically, I use the color light gray for a strength of association above average, medium gray for a strength of association within the 95 perc. asymptotic confidence interval of the mean, and dark gray for a strength of association significantly below average. <br>
...
Specifically, the range of the strength of association on the basis of Spearman's $\rho$ (Kendall's $\tau$) is between 0.51 (0.40) for Germany and 0.96 (0.86) for Ireland. <br>
...
plot.wiod.RCA.EXGR.FVAX.df$kend.cat3<-Hmisc::cut2(plot.wiod.RCA.EXGR.FVAX.df$kendall,cuts=c(kend.cats.low.up[1],kend.cats.low.up[2],max(plot.wiod.RCA.EXGR.FVAX.df$kendall)))  
#reverse factor
...
p.kendall.exgr.fvax<-ggplot(plot.wiod.RCA.EXGR.FVAX.df, aes(x=reorder(COU,-kendall), y=kendall)) + 
  geom_bar(stat="identity", position="dodge", color="black",aes(fill = kend.cat3)) +
...
plot.wiod.RCA.EXGR.FVAX.df$spear.cat3<-Hmisc::cut2(plot.wiod.RCA.EXGR.FVAX.df$spearman,cuts=c(spear.cats.low.up[1],spear.cats.low.up[2],max(plot.wiod.RCA.EXGR.FVAX.df$spearman)))  
#reverse factor
...
```
```{r fig Strength of association: EXGR FVAT RCA, echo=FALSE, fig.alig='center', fig.cap='Strength of association: EXGR & FVAT RCA'}
...
The RCA ranking obtained on the basis of the factor content of trade is substantially different from the ranking obtained on the basis of EXGR.<br>
...
I test the hypothesis that differences in the strength of association may be connected to the country's level of development.<br>
...
```{r cor RCA on the basis of R regression, cache=T,include=FALSE, message=FALSE, warning=FALSE, cache=T,cache.lazy=FALSE}
GDP.cntry<-readr::read_csv(file="GDP_per_capita_World_Development_Indicators_Data.csv",na = "..")
...
```
```{r data prep fig 2-6 2-7, message=FALSE, warning=FALSE, cache=T, message=FALSE, warning=FALSE, cache=T, include=FALSE,cache.lazy=FALSE}
...
```{r WIOD STRENGTH OF ASSOCIATION GDP PER CAPITA,cache=T, message=FALSE, warning=FALSE, cache=T, include=FALSE,cache.lazy=FALSE }
...
```{r plot Strength of association and GDP per capita, fig.align='center', fig.cap='Strength of association and GDP per capita', echo=FALSE}
grid.arrange(p.wiod.tiva.fvax.exgr.gdp.sp,p.wiod.tiva.fvax.exgr.gdp.kend,top="Strength of association and GDP per capita",ncol=2)
```
...
I focus on the set of 34 countries present in both databases. <br>
Figure 2.6 and figure 2.7  summarize the differences between the strength of association that I obtain in each dataset.
The figures highlight that there is a substantial degree of divergence between RCA rankings obtained in each dataset.<br>
...
Further, the countries are sorted in each group according to the absolute difference of the strength of association. <br>
...
```{r FVAT EXGR RCA - WiOD and TiVA, message=FALSE, warning=FALSE, cache=T, cache.lazy=FALSE, include=FALSE}
...
#categorize
stata.r.fvax.gdp.fvax.RCA$R.RCA$RCA.FVAX.kend$diff.categories<-Hmisc::cut2(stata.r.fvax.gdp.fvax.RCA$R.RCA$RCA.FVAX.kend$diff.wiod.tiva,cuts=c(-0.40,-0.05,0.05,max(stata.r.fvax.gdp.fvax.RCA$R.RCA$RCA.FVAX.kend$diff.wiod.tiva)))
...
#categorize spearman
stata.r.fvax.gdp.fvax.RCA$R.RCA$RCA.FVAX.spear$diff.categories<-Hmisc::cut2(stata.r.fvax.gdp.fvax.RCA$R.RCA$RCA.FVAX.spear$diff.wiod.tiva,cuts=c(-0.40,-0.05,0.05,max(stata.r.fvax.gdp.fvax.RCA$R.RCA$RCA.FVAX.spear$diff.wiod.tiva)))
...
```{r FVAT EXGR RCA - WiOD and TiVA prep plots, cache=T, cache.lazy=FALSE, include=FALSE}
...
```{r FVAT EXGR RCA - WiOD and TiVA plots, cache=T, cache.lazy=FALSE, include=FALSE}
facet_names <- c(
    "[-0.400,-0.050)"="Decreased",
    "[-0.050, 0.050)"="Unchanged" ,
   "[ 0.050, 0.325]"="Increased" 
)
p.wi.ti.fvax.exgr.kend<-ggplot(stata.r.fvax.gdp.fvax.RCA$R.RCA$RCA.FVAX.kend, aes(x=kendall, y=COU, shape=variable)) + 
  ...
  scale_color_brewer(palette="Set1",labels=c("[-0.400,-0.050)","[-0.050, 0.050)","[ 0.050, 0.325]"))+
...
clev.small.multiple<-ggplot(stata.r.fvax.gdp.fvax.RCA$R.RCA$RCA.FVAX.kend, aes(x=kendall, y=COU, shape=variable)) + 
  ...
  scale_shape_manual(values=c(16,17))+
  scale_color_manual(name="",values = c("#edf8b1","#7fcdbb","#2c7fb8"),labels=c("[-0.40,-0.05)","[-0.05, 0.05)","[ 0.05, 0.32]"))+#labels=c("[-0.39,-0.05)","[-0.05, 0.05)","[ 0.05, 0.42]"))+
  coord_cartesian(xlim=c(0.00,1))+
  scale_x_continuous(breaks=c(seq(0,1,0.1)),minor_breaks=c(seq(0.05,1,.1))) +
  labs(y="",x=expression(paste("Kendall's ",tau)))+
  guides(colour=guide_legend(title=expression(paste(tau["TiVA"]," -- ", tau["WiOD"])),nrow=2,byrow=T,order = 1,override.aes = list(shape = 15)),shape=guide_legend(title="",order = 2,label.vjust=0))+
  theme_minimal()+
    theme(#axis.title = element_blank(),
              panel.grid.major.x = element_blank(),
              panel.grid.minor = element_blank(),
...
clev.spear<- ggplot(stata.r.fvax.gdp.fvax.RCA$R.RCA$RCA.FVAX.spear, aes(x=spearman, y=COU, shape=variable)) + 
    ...
    #scale_color_brewer(palette="Set1",labels=c("[-0.40,-0.05)","[-0.05, 0.05)","[ 0.05, 0.30]"))+
    scale_shape_manual(values=c(16,17))+
   scale_color_manual(name="",values = c("#edf8b1","#7fcdbb","#2c7fb8"),labels=c("[-0.40,-0.05)","[-0.05, 0.05)","[ 0.05, 0.42]"))+
  ...
  guides(colour=guide_legend(title=expression(paste(rho["TiVA"]," -- ", rho["WiOD"])),nrow=2,byrow=T,order = 1,override.aes = list(shape = 15)),shape=guide_legend(title="",order = 2,label.vjust=0))+
    theme_minimal()+
    theme(#axis.title = element_blank(),
              panel.grid.major.x = element_blank(),
              panel.grid.minor = element_blank(),
          ...
          plot.title = element_text(size = 20, margin = margin(b = 10)))+
              ...
              #plot.subtitle = element_text(size = 10, color = "darkslategrey", margin = margin(b = 25)),
              # plot.caption = element_text(size = 8, margin = margin(t = 10), color = "grey70", hjust = 0))+
  facet_grid(diff.categories~.,   labeller = as_labeller(facet_names.spear),scales ="free_y",space="free_y")
    # theme( legend.position = "bottom", # legend location in graph
    #        # panel.grid.minor = element_blank(),
    #        panel.grid.major = element_line(size = 0.7),
    #        panel.grid.minor = element_line(size = 0.5),
...
```
```{r fig 2-6 FVAT EXGR RCA WiOD and TiVA, echo=FALSE, fig.align='center', fig.cap='Strength of association: RCA - FVAT and EXGR - WiOD and TiVA', fig.height=10, fig.width=10, message=FALSE, warning=FALSE}
grid.arrange(clev.small.multiple ,clev.spear,ncol=2,top='Strength of association: RCA - FVAT and EXGR - WiOD and TiVA' )
```


I investigate whether the discrepancies between the strength of the association of the two rankings may be related to a country's level of development, as measured by its GDP per capita. <br> 
...
#ggplot(wiod.tiva.fvax.exgr.gdp.kend.wide, aes(x=COU,y=diff.wiod.tiva))+
  ...
  scale_y_continuous(breaks=c(-0.05,0.05,0.15,0.25,0.35,0.45))+
...
#dev.off
```
```{r fig 2-7 plot distnce strength of associations of RCA rankings, echo=FALSE, fig.align='center', fig.cap='Euclidian distance: Strength of associations of RCA rankings'}
grid.arrange(plot.wiod.kend.gdp.eu,plot.wiod.t.gdp.euc, top="Distance strength of associations of RCA rankings -- WiOD and TiVA",ncol=2)
```
...
more on the intrinsic efficiency of the domestic production factors, such as the USA and Ireland. <br>
...
library(boot)
```
```{r prep eigenvec, message=FALSE, warning=FALSE, cache=T, include=FALSE}
...
```
```{r plot cor RCA Eigen, cache=T, include=FALSE}
################################################################PLOT RESULTS OF COR EIGEN RCA ################################################################
results.spearman$cat.4<-Hmisc::cut2(results.spearman$spearman,c(-0.139,0,0.7,0.888,.989), g=4)
...
p.eigen.RCA.spear<-ggplot(results.spearman, aes(reorder(COU,-spearman),spearman,fill=cat.4))+ 
 # geom_errorbar(aes(ymin=ci.min.spear.fiel, ymax=ci.max.spear.fiel), colour="black", width=.1) +
  ...
  labs(y=expression(paste("Strength of Association - Spearman's ",rho)))+
...
results.kendall$cat.4<-Hmisc::cut2(results.kendall$kendall,c(-0.13,0,round(outliner.kend[1],3),round(median(results.kendall$kendall),3),round(max(results.kendall$kendall),3)),g=4)
...
Each edge $g_{i,j}$ is recorded in the adjacency matrix $\boldsymbol{G}$ of the dimensions $n \times n$.<br>
The binary trade network is the tuple of nodes and edges $(N, \boldsymbol{G})$. <br>
...
<br> All the weights are recorded in the weight matrix $\boldsymbol{W}$ of the dimensions $n \times n$. 
The weighted international trade network is the triplet of the set of countries, the matrix of trade relationships and the weight matrix, thus  $ITN =( (N, \boldsymbol{G}), \boldsymbol{W})$. <br>
...
The focus of network theory are relationships among nodes, thus their interactions, while taking into account the complete set of the effects of other nodes on these interactions <br>encite{de2014network].
...
The equation 3.1 may be restated in matrix notation as below. It is the general eigenvector equation and  it  has $n$ solutions for $n$ values of $\lambda$. 
\begin{align}
\label{eq:6}
...
Second, it implies that the left-hand eigenvector of the largest eigenvalue is positive for a non-negative row stochastic matrix.<br>
I focus on the principal eigenvector corresponding to the largest eigenvalue as the measure of network centrality. It is this eigenvector to which the matrix of trade relationships converges when taken to the power of $n$. <br> 
Accordingly, equation 3.2 simplifies to the following equation, where $\boldsymbol{c}^e$ denotes the principal eigenvector.
\begin{align}
\label{eq:17}
...
The weighted out-eigenvector centrality is then the right hand eigenvector of the transposed matrix $\boldsymbol{A}^T$. <br>
...
Therefore, a shock to a country with a higher centrality has larger effects on the international trade network. <br>
...
Second, the definition of the international trade networks on the basis of forward value-added trade, allows to correctly identify how shocks to a country's domestic production factors propagate through the international trade network. <br>
...
This group includes nearly three quarters of the countries in the sample. <br>
...
```{r shock propagation countries, message=FALSE, warning=FALSE, cache=T, include=FALSE}
...
p.7.4<-ggplot(upper7_4,aes(x=IND,y=rank, group=COU,color=COU,shape=COU))+
  ...
  scale_y_continuous(breaks=seq(0,14,1),expand=c(0.02,0))+
  scale_shape_manual(values=c(seq(4,7,1)))+
  scale_color_manual(values=col.up.7.2[1:4])+
  theme_minimal()+
  labs(y="Rank",x="IND", title="")+
    theme(#axis.title = element_blank(),
              panel.grid.major.x = element_blank(),
              panel.grid.minor = element_blank(),
              legend.title = element_blank(),
...
p.7.3<-ggplot(upper7_3,aes(x=IND,y=rank, group=COU,color=COU,shape=COU))+
  ...
  scale_y_continuous(breaks=seq(0,14,1),expand=c(0.02,0))+
  scale_shape_manual(values=c(seq(4,7,1)))+
  scale_color_manual(values=col.up.7.2[5:7])+
  theme_minimal()+
  labs(y="Rank",x="IND", title="")+
   theme(#axis.title = element_blank(),
              panel.grid.major.x = element_blank(),
...
#dev.off()
```
```{r shock importance of countries, message=FALSE, warning=FALSE, cache=T, include=F}
tiva.2005.agg.cou.par<-tiva.2005.sub.final %>% filter(.,VAR%in%c("FFD_DVA")) %>% group_by(COU,PAR) %>% summarise(sum.fvat.ind=sum(Value))
 ...
 p.1.out.eigen.cou.par<- ggplot(eigen.res.cou.par.tb, aes(reorder(COU, -out.eigen.cent),out.eigen.cent,fill=cor.pres)) +
   ...
   # geom_point(aes(color=cor.pres))+ 
   labs(x="",y="",title="Out eigenvector centrality",subtitle ="")+
   scale_fill_manual(values=c("#999999","#000000"))+
    #scale_fill_brewer(type="seq",palette =6)+
   # scale_color_brewer()+
   scale_x_discrete( expand = c(0.00, 0.00))+
    scale_y_continuous(breaks=c(seq(0,1,.1)),minor_breaks=c(seq(0.05,1.0,.1)), expand = c(0.02, 0.00))+
    theme_minimal()+
  theme(axis.title.x = element_blank(),
              panel.grid.major.x = element_blank(),
              panel.grid.minor = element_blank(),
...
```
```{r plot shock propagation countries, echo=FALSE, fig.align='center', fig.cap='Shock propagation: Importance of countries', message=FALSE, warning=FALSE}
...
The right panel shows the remaining three countries of the core group. <br>
...
For example, China is ranked first in the industry 'textiles and leather' industry (17-19) and is among the ten most important countries across the manufacturing industries. Yet, its rank position is only thirteenth in the 'financial services and intermediation' industry (65-67) and eleventh in the 'real estate and business services' industry (70-74). <br>
...
```{r shock propagation, echo=FALSE, fig.align='center', fig.cap='Shock propagation: Seven highest ranked countries across industries', fig.width=12, message=FALSE, warning=FALSE}
grid.arrange(p.7.4,p.7.3,nrow=1,ncol=2,top='Shock propagation: Seven highest ranked countries across industries')
```
...
```{r plot eigen RCA, echo=FALSE, fig.align='center', fig.cap='Strength of association between RCA and eigenvector centrality', fig.width=12, message=FALSE, warning=FALSE}
grid.arrange(p.eigen.RCA.spear,p.kend.RCA.eigen,ncol=2,top='Strength of association between RCA and eigenvector centrality')
```
...
Specifically, I  compared the RCA ranking obtained on the basis of gross exports to the RCA rankings on the basis of backward value-added trade and of forward-value-added trade. <br>
...
However, there are noticeable differences between the ranking of RCA on the basis of the factor content of trade to the ranking on the basis of gross exports. <br>
...
However, for six countries I find that ranking of relative network centrality does not map into their pattern of RCA. <br>
...
Finally, I discuss the results. <br>
...
In the equation $x_{i,j}^k$ denotes the trade flows between exporting country $i$, importing country $j$ in industry $k$, $\delta_{i,j}$ denotes an importer-exporter fixed effect, $\delta_{j}^k$ denotes an importer-sector fixed effect,  $z_{i,j}^k$ denotes the productive efficiency and $\epsilon_{i,j}^k$ denotes the error term.  As in CDK I specify $z_{i,j}^k$ as the inverse of producer prices. <br>
...
Thus, the exogeneity condition requires for the three specifications that the inverse of total production cost, total domestic cost and total domestic factor cost is not correlated with variable trade cost. <br>
...
The sign of the simultaneity bias is apriori ambiguous @costinot}. <br>
Second, the exogeneity condition may be violated due to a measurement error.
A measurement error of the international price data would downward bias the estimate of $\theta$ under the condition that the measurement error is correlated with the true underlying variable [@greene,p.85].<br>
...
<br>%In the next paragraph, I discuss the consequences if the second assumption is violated.<br>
...
In the model a decrease in the cost of producing a good is directly passed through to the producer price, due to  the assumption of perfect competition. <br>
...
In particular, the missing data may cause a loss of efficiency in the first stage of the IV estimation, which would reduce the strength of the first stage association between R&amp;D and the inverse of producer prices and hence upward bias the estimates of $\theta$. <br>

Multiple imputation is a Bayesian technique to impute missing data by simulated draws from the posterior predictive distribution. ^[It was initially proposed in [@rubin1978] for non-response in surveys, and its statistical properties were developed in  [@Rubin1987].]<br>
 In the following, I outline multiple imputation based on  [@Little:2002a, pp. 209-211].<br>
...
Multiple imputation as other missing data imputation techniques assume that the probability of a missing observation depends only on the observed data and not on the missing data. <br>
...
<br>
Multiple imputation produces valid interference from a frequentist perspective [@Little:2002a,p.90]. <br>
...
This choice rests on the recommendations of the simulation study by [@Morris2014].<br> 
...
The significant increase of the IV estimate compared to the OLS  estimate, confirms that the regressor is endogenous [@hausman1978].<br>
...
A higher estimate of $\theta$ implies a decreased dispersion of production costs within sectors. The result is in line with our expectations, since the sample includes only high income countries.<br>
...
Directly comparing the estimate of $\theta$ on the basis of gross exports to the result of @costinot}, I find that the IV results in column 2-3 are not statistically different from CDKs results  ($\theta_{CDK}$ 11.1 SE 0.981).
  ...
  The authors use openness corrected gross exports to account for trade selection, which  downward biases  the productive differences. ^[Trade selection denotes that a country does not produce certain goods for which they receive a low productivity draw and instead import them [@costinot].] <br>
...
```{r theta, message=FALSE, warning=FALSE, cache=T, include=FALSE}
ov.caption="Estimation of theta"
cgroup.1<-c("Dependent variable log gross exports in 2005")
cgroup.2<-c("Dependent variable log backward value-added in 2005")
cgroup.3<-c("Dependent variable log forward value-added in 2005")
...
exge.se<-c(0.067,1.331,1.422,2.13) 
Exp.Imp.FE<-rep("Yes",4)
Imp.Sec.FE<-rep("Yes",4)
r.sq.exgr<-c( 0.771, 0.197 , 0.321, 0.141)
rownames.3<-c("Log inverse producer prices","","Exporter-Importer Fixed Effect","Exporter-Sector Fixed Effect", "Observations","First-Stage F-Statistic")
exgr.obs<-c(18143,18143,16582,14449)
f.stat<-c( NA,151.41,     125.60      ,    85.24 )

bl.tb.cgroup.1<-c("Heteroscedasticity robust standard errors in parentheses")
bl.tb.cgroup.2<-c("Log Productivity is instrumented in columns 2-6 with log of R&amp;D expenditures")
...
The results of the first stage regression address two concerns about the validity of the IV regression: the relevance of the instrument and whether the instrument affects the endogenous regressor in the hypothesized way. <br>
...
kable(table.1)
# \begin{table}[H]\centering\caption{Summary statistics in $\theta$ sample}
# \footnotesize
# \label{tab:sumstat}
# \begin{tabular}{l *{5}{c}}\toprule
# \multicolumn{1}{c}{\textbf{Variable}} & \textbf{Mean}
#  & \textbf{Std. Dev.}& \textbf{Min.} &  \textbf{Max.} & \textbf{N}\\ \midrule
# Log BVAT&  2.443 & -4.605  & 10.754&2.867  & 17453\\
# Log EXGR& 2.742 & 2.871  & - 4.605  & 11.10807& 17505\\
# Log FVAT& 3 & 2.351 & - 4.605 & 10.739 & 15999\\
# Log inv. prod. prices & 0.267 &   0.274&  -.672  & 1.167 & 18444\\
